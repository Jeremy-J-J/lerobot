...
class LeRobotDatasetMetadata:
    def __init__(
        ...
        try:
            ...
            # Offline/local-only mode: do not fall back to Hugging Face when
            # local metadata is missing.
            raise FileNotFoundError(
                f"Local dataset metadata was not found at {self.root / INFO_PATH}. "
                "Create the dataset first, pass the correct local root, or run with resume=false."
            ) from e
    ...
    def create(
        ...
        if obj.root.exists():
            if any(path.is_file() for path in obj.root.rglob("*")):
                raise FileExistsError(
                    f"Dataset root {obj.root} already contains files. "
                    "Use resume=true to continue an existing dataset or choose a new repo_id."
                )
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        try:
            ...
            # Offline/local-only mode: do not download dataset files from
            # Hugging Face when local parquet/video files are missing.
            raise FileNotFoundError(
                f"Local dataset files are incomplete under {self.root}. "
                "Provide a complete local dataset or create a new one with resume=false."
            ) from e
